...
export const createTailwindConfig = async (answers: TAnswers) => {
  ...
  await execCommandOnProject(answers)(
    `${
      tailwindInstallCommands[answers.packageManager]
    } && npx tailwindcss init -p`
  );

  await execCommandOnProject(answers)(
    `echo "${tailwindConfigString}" > tailwind.config.js`
  );

  if (answers.projectFramework === "next.js") {
    await execCommandOnProject(answers)(
      `echo "${tailwindClassCss}" > ./src/styles/globals.scss`
    );
  } else if (answers.projectFramework === "@angular") {
    await execCommandOnProject(answers)(
      `echo "${tailwindClassCss}" > ./src/styles.scss`
    );
  }
